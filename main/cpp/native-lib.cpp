...
Java_com_vgram_demo_ImageActivity_Stitch(JNIEnv *env, jobject obj, jobjectArray source, jstring result, jdouble scale, jboolean check);
...
Java_com_vgram_demo_ImageActivity_Stitch(JNIEnv *env, jobject obj, jobjectArray source, jstring result, jdouble scale, jboolean check) {
    ...
    if (isCheck) {
        for (int i = 0; i < imgCount; i++) {
            ...
            LOGE("IsCheck, Add index %d source_name=:%s", i, source_name);
            ...
            resize(img_rotated, img_scaled, dsize);
