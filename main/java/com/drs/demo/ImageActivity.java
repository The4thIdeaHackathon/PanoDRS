...
public class ImageActivity extends Activity implements View.OnClickListener, BottomNavigationView.OnNavigationItemSelectedListener {
    ...
    private final String STITCHING_RESULT_IMAGES_DIRECTORY = Environment.getExternalStorageDirectory().getPath() + "/PanoDRS/";
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_check: {
                if(!isCheck) {
                    isCheck = true;
                    mBtnCheck.setBackground(getResources().getDrawable(R.drawable.btn_check));
                    showToast("회전 활성화");
                }
                else {
                    isCheck = false;
                    mBtnCheck.setBackground(getResources().getDrawable(R.drawable.btn_uncheck));
                    showToast("회전 비활성화");
                }
            }
    ...
    private void startStitch() {
        new Thread() {
            ...
            public void run() {
                ...
                if(Stitch(source, STITCHING_RESULT_IMAGES_DIRECTORY + "Panorama.jpg", STITCH_IMAGE_SCALE, isCheck) == 0) {
                    showToast("스티칭 성공!\n걸린시간 : " + stitchCost);
                }else{
                    showToast("스티칭 실패..");
                }
     ...
     */
    public native int Stitch(String[] source, String result, double scale, boolean check);
